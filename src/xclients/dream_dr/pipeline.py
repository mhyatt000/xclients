...
from xclients.dream_dr.arrayrecord_extrinsics import materialize_arrayrecord_extrinsics, MaterializeConfig
...
def main_arrayrecord(cfg: Config) -> None:
    ...
    if ht_by_camera:
        ht_path = cfg.output_dir / "HT_dr_cameras.npz"
        np.savez_compressed(
            ht_path,
            **{camera: np.asarray(ht, dtype=np.float32) for camera, ht in ht_by_camera.items()},
        )
        maybe_write_arrayrecord_extrinsics(cfg, ht_path)
    if wrote_outputs:
        logging.info("Wrote arrayrecord outputs to %s", cfg.output_dir)


def maybe_write_arrayrecord_extrinsics(cfg: Config, ht_path: Path) -> None:
    if cfg.arrayrecord_output_version is None and cfg.arrayrecord_output_dir is None:
        return
    if cfg.arrayrecord_dir is None:
        raise ValueError("arrayrecord_dir is required to write ArrayRecord extrinsics")

    logging.info(
        "Materializing ArrayRecord extrinsics from %s to version=%s dir=%s",
        ht_path,
        cfg.arrayrecord_output_version,
        cfg.arrayrecord_output_dir,
    )
    materialize_arrayrecord_extrinsics(
        MaterializeConfig(
            arrayrecord_dir=cfg.arrayrecord_dir,
            extrinsics_npz=ht_path,
            output_arrayrecord_dir=cfg.arrayrecord_output_dir,
            output_version=cfg.arrayrecord_output_version,
            overwrite=cfg.arrayrecord_output_overwrite,
        )
    )
